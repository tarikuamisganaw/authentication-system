...
UserSchema.methods.generateResetToken = async function () {
  const resetTokenValue = crypto.randomBytes(20).toString("base64");
  const resetTokenSecret = crypto.randomBytes(10).toString("hex");
  const user = this;

  // Make resetTokenValue URL-safe
  const urlSafeResetTokenValue = resetTokenValue
    .replace(/\+/g, "-")
    .replace(/\//g, "_")
    .replace(/=+$/, "");

  const resetToken = `${urlSafeResetTokenValue}+${resetTokenSecret}`;

  const resetTokenHash = crypto
    .createHmac("sha256", resetTokenSecret)
    .update(urlSafeResetTokenValue)
    .digest("hex");

  user.resetpasswordtoken = resetTokenHash;

  // Ensure RESET_PASSWORD_TOKEN.expiry is defined and valid, defaulting to 5 minutes if undefined
  const expiryInMinutes = parseInt(RESET_PASSWORD_TOKEN.expiry, 10) || 5;
  user.resetpasswordtokenexpiry = new Date(Date.now() + expiryInMinutes * 60 * 1000);
